...
First, you'll want to make a new Stream and attach your sinks to it.
Streams are commonly stored in a global variable.

```go
import (
	"github.com/pchchv/health"
	"github.com/pchchv/health/sinks/bugsnag"
	"os"
)
...
health excels at instrumenting services that perform *jobs*. Examples of jobs: serving an HTTP request, serving an RPC request, or processing a message from a work queue. Jobs are encoded semantically into pchchv/health in order to provide out-of-the-box answers to questions like, "what is my slowest endpoint?"
...
```
[2024-03-11T22:53:22.115855203Z]: job:/api/v2/user_stories event:starting_request kvs:[path:/api/v2/user_stories request-id:F8a8bQOWmRpO6ky]
```
...
Since error handling is so prevalent in Go code, you'll have sitations where multiple functions have the option of loggin the same root error. The best practice that we've identified is to just not think about it and log it on every level of the call stack. Keep in mind that pchchv/health will handle this intelligently and only send one error to Bugsnag, have a correct root backtrace, and so on.
...
```bash
go get github.com/pchchv/health/cmd/healthd
```
...
```bash
go get github.com/pchchv/health/cmd/healthtop
```
...
2 -- if this is not the case, you can use the source option: ```healthtop --source=10.28.3.132:5032 jobs```)
